...
class Bookmark extends Organizr
{
	...
	public function _getSettings()
	{
		return array(
		    'Options' => array(
                array(
                    'type' => 'switch',
                    'name' => 'bookmarksTargetBlank',
                    'label' => 'bookmarks Target to Blank',
                    'value' => $this->config['bookmarksTargetBlank']
                )
            ),
			'custom' => '
				<div class="row">
	...
	public function _getPage()
	{
		...
		foreach ($this->_getAllCategories() as $category) {
			...
			foreach ($tabs as $tab) {
				$bookmarks .= '<a href="' . $tab['url'] . '" target="'.($this->config['bookmarksTargetBlank']?'_blank':'_SELF').'">
